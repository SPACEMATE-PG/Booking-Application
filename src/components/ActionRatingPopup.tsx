...
import { useUser } from "@/lib/user-context"; // Assuming Clerk for user context
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "sonner";

export function ActionRatingPopup() {
    const { isOpen, hideRating, actionName, metadata } = useRating();
    const { user } = useUser(); // Need user context to get ID
    ...
    const handleSubmit = async () => {
        ...
        if (!metadata.propertyId || !metadata.bookingId) {
            // If no metadata, just close (e.g. for generic feedback)
